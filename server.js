...
app.get("/health", (_req, res) => {
  res.json({ ok: true, players: players.size, state: game.state });
});

function getLocalIPv4Urls() {
...
server.listen(PORT, HOST, () => {
  ...
  const lanUrls = getLocalIPv4Urls();
  if (lanUrls.length > 0) {
    console.log("Open from other devices on the same Wi-Fi using one of these URLs:");
